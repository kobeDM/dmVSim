...
int main(int argc,char **argv)
{
    ...
    std::cout << "dmM="     << dmM  << " signal="                            << log10f(snum) << std::endl;
    std::cout << "r="       << r    << endl;
    ...
    double *x; // x bin
    double vdm;
    double nMMtrx[15]; // nuclear mass matrics
    ...
    //--------- Nucelar Mass Matrix -----------//
    // sahigash: what is 0.932 [GeV]? I guess it should be 0.938 or 0.939 [GeV] which corresponds to the proton (neutron) mass
    //SI
    nMMtrx[0] = 0.932*(0.989*12.+0.011*13);		//C
    nMMtrx[1] = 0.932*(0.950*32.+0.008*33.+0.042*34.);	//S
    nMMtrx[2] = 0.932*(0.5069*79.+0.4931*81.);		//Br
    nMMtrx[3] = 0.932*127.;				//I
    nMMtrx[11]= 0.932*(0.5184*107.+0.48161*109.);	//Ag
    //SD
    nMMtrx[10]= 0.932*19.;				//F
    ...
    std::stringstream s_dmM;
    std::stringstream s_atom;
    std::stringstream s_r;
    s_dmM << dmM; s_atom << atom; s_r << r;
    std::string filename=filename_head + s_dmM.str() + "_" + s_atom.str() +"_" + s_r.str()+ filename_ext;
    std::string filename_ext_info=".info";
    std::string filename_info=filename_head + s_dmM.str() + "_" + s_atom.str() +"_" + s_r.str()+ filename_ext_info;
    ...
    while( j < snum ){
        ...
        Ctheta1=fCtheta1(dmM,mn,CT);
        Ctheta2=fCtheta2(CT);
        Udm=mn/(dmM+mn)*vdmE0;
        Un=dmM/mn*Udm;
        udm=fudm(V,Udm,CT,Ctheta1);
        un=fun(vdmE0,udm,dmM,mn);
        //Sphi1=0.;
        //Sphi2=-Sphi1;
        Sphi1=fSphi1();
        Sphi2=-Sphi1;
        ...
        if((long double) (rand()/(RAND_MAX+1.0)) <= FF2(ER(mn,dmM,norm(unE)),atom)){

            fprintf(fp1,"%d %d %f %f %f %d\n",
                    r,atom,Ct2E,Sp2E,ER(mn,dmM,norm(unE)),vflg);
            j++;
            if(vflg!=0&&vflg!=1)
                printf("%d %d %f %f %f %d\n",
                       r,atom,Ct2E,Sp2E,ER(mn,dmM,norm(unE)),vflg);
