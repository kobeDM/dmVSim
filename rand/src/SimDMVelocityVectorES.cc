...
int main( int argc, char** argv )
{
    if( argc != 6 ) {
        ...
        std::cerr << "./SimDMVelocityVectorES [l.o.s. (kpc)] [DM mass (GeV)] [profile (NFW or IT or EIN)] [The number of events] [output filename]" << std::endl;
        abort( );
    }
    ...
    if( profile == "NFW" ) {
        pFuncFlux = new TF3( "flux", getDMNFWFluxIntVBES, -0.5*TMath::Pi( ), 0.5*TMath::Pi( ), 0.0, 2.0 * TMath::Pi( ), 1e-8, 1e+12, 13, 3 );
    ...
    TF1* pFuncEn = new TF1( "fluxEn", getDMFluxEnVBES, 0.0, V_LIGHT, 9, 1 );

    double g_DM_V = 1.0, g_p_V = 1.0, g_n_V = 1.0;
    double A = 1, Z = 1; // assuming proton only
    double VM = 91.19; // Z boson mass (GeV)
    pFuncEn->SetParameter( 0, g_DM_V      );
    pFuncEn->SetParameter( 1, g_p_V       );
    pFuncEn->SetParameter( 2, g_n_V       );
    pFuncEn->SetParameter( 3, A           );
    pFuncEn->SetParameter( 4, Z           );
    pFuncEn->SetParameter( 5, VM          );
    pFuncEn->SetParameter( 6, PROTON_MASS );
    pFuncEn->SetParameter( 7, dmM         );
    pFuncEn->SetParameter( 8, LAMBDA_P    );
    ...
    pFuncFlux->SetParameter( 1,  rhoScale     );
    pFuncFlux->SetParameter( 2,  rScaleKPC    );
    pFuncFlux->SetParameter( 3,  SUN_DISTANCE );
    pFuncFlux->SetParameter( 4,  g_DM_V       );
    pFuncFlux->SetParameter( 5,  g_p_V        );
    pFuncFlux->SetParameter( 6,  g_n_V        );
    pFuncFlux->SetParameter( 7,  A            );
    pFuncFlux->SetParameter( 8,  Z            );
    pFuncFlux->SetParameter( 9,  VM           );
    pFuncFlux->SetParameter( 10, PROTON_MASS  );
    pFuncFlux->SetParameter( 11, dmM          );
    pFuncFlux->SetParameter( 12, LAMBDA_P     );
